    ...
    (data.topics || []).forEach((topic) => {
      ...
      (topic.panels || []).forEach((panel) => {
        ...
        (panel.items || []).forEach((item) => {
          ...
          li.innerHTML = `
            <a class="headlineLink" href="${href || '#'}" ${href ? 'target="_blank" rel="noopener noreferrer"' : ''}>${safeTitle}</a>
            <div class="sourceLine">Source: ${sourceName}${publishedAt ? ` · ${publishedAt}` : ""}</div>
          `;
